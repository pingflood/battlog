...
pc:
	gcc battlog.c -g -o battlog.dge -lm -ggdb -O0 -DDEBUG -lSDL_image -lSDL -lSDL_ttf -I/usr/include/SDL

retrogame:
	$(CXX) $(CFLAGS) $(LDFLAGS) battlog.c -o battlog.dge
...
ipk: retrogame
	...
	@cp -r battlog.dge battlog.png /tmp/.battlog-ipk/root/home/retrofw/apps/battlog
	@cp battlog.lnk /tmp/.battlog-ipk/root/home/retrofw/apps/gmenu2x/sections/applications
	@sed "s/^Version:.*/Version: $$(date +%Y%m%d)/" control > /tmp/.battlog-ipk/control
	@tar --owner=0 --group=0 -czvf /tmp/.battlog-ipk/control.tar.gz -C /tmp/.battlog-ipk/ control
	@tar --owner=0 --group=0 -czvf /tmp/.battlog-ipk/data.tar.gz -C /tmp/.battlog-ipk/root/ .
	@echo 2.0 > /tmp/.battlog-ipk/debian-binary
	@ar r battlog.ipk /tmp/.battlog-ipk/control.tar.gz /tmp/.battlog-ipk/data.tar.gz /tmp/.battlog-ipk/debian-binary

clean:
	rm -rf battlog.dge battlog.ipk
